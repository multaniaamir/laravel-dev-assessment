...
// Initialize the form with title and location fields
const form = useForm({
  title: '',
  location: '',
});
...
// Reactive inputs for two-way binding
const titleInput = ref('');
const locationInput = ref('');

// Sync inputs with form data
const syncInputs = () => {
...
<template>
  <div class="bg-slate-100 relative">
    <div class="container py-32 text-center flex flex-col gap-8 relative">
      ...
      <!-- Search -->
      <div>
        <form @submit.prevent="submit" class="bg-white w-full border rounded-full overflow-hidden border-gray-200 max-w-3xl mx-auto flex items-center justify-center">
          ...
          <button type="submit" class="bg-brand px-6 text-sm font-medium py-2 rounded-full text-white mr-3">
            Find jobs
          </button>
        </form>
      </div>
